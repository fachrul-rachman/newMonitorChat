...
- Halaman utama yang diakses user:
  - `/login` → halaman login internal.
  - `/` → Dashboard (ringkasan sesi, response time, sesi pending, kata terbanyak).
  - `/chat` → Chat Viewer (daftar sesi + tampilan chat mirip WhatsApp, auto-refresh).
- Secara internal Next.js tetap merender dari route `/monitorchat/*` melalui konfigurasi rewrite.
...
4. Buka `http://localhost:3000/login` lalu login dengan credential sesuai `.env`.
